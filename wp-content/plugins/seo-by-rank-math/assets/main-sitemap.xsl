			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Ubicaciones del mapa del sitio - La Terracita Encantada</title>
					</xsl:when>
					<xsl:otherwise>
						<title>Mapa del sitio XML - La Terracita Encantada</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="http://la-terracita-encantada.local/sitemap_index.xml">&#8592; Índice del mapa del sitio</a>							</p>
